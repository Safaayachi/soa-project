...
const productService = {
  getProduct: async (call, callback) => {
    ...
    try {
      const product = await client.db().collection("products").findOne({ id: productId });
  ...
  searchProducts: async (call, callback) => {
    ...
    try {
      const products = await client.db().collection("products").find({ $text: { $search: query } }).toArray();
  ...
  createProduct: async (call, callback) => {
    ...
    try {
      const result = await client.db().collection("products").insertOne(product);
      callback(null, { product });
    } catch (err) {
      callback(err);
    }
